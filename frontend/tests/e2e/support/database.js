...
Cypress.Commands.add('cleanItemCombinationQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='item_combination' where title like '%${questionName}%')
                , itemToDelete AS (SELECT id FROM  item_combination_item WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
                , opt AS (DELETE FROM  item_combination_options WHERE item_combination_id IN (SELECT id FROM itemToDelete))
                , item AS (DELETE FROM  item_combination_item WHERE id IN (SELECT id FROM itemToDelete)) 
                , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
              DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});

// this function doesnt work, we have an hotfix above
...
/*Cypress.Commands.add('cleanOpenAnswerQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='open_answer' where title like '%${questionName}%')
                , correctToDelete AS (SELECT id FROM  item_combination_item WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
