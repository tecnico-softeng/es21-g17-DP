<template>
  <ul>
    <li v-for="option in questionDetails.options" :key="option.id">
      <span
        v-if="option.correct && questionDetails.relevanceOfOptionsConsidered"
        v-html="
          convertMarkDown(
            studentAnswered(option.id) + 'Relevancy  ' + option.relevance + ' **[★]** ' + option.content
      ...
      />
      <span
        v-else-if="option.correct && !questionDetails.relevanceOfOptionsConsidered"
        v-html="
          convertMarkDown(
