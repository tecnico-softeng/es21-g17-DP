...
public abstract class LatexVisitor implements Visitor {
    ...
    public void visitQuestionDetails(MultipleChoiceQuestion question) {
        ...
        this.result = this.result + "\\relevanceConsidered{" + question.isRelevanceOfOptionsConsidered() + "}\n";
    ...
    public void visitQuestionDetails(ItemCombinationQuestion question) {
        ...
        int i = 0;
        for(ItemCombinationItem it: question.getItemCombinationItems()) {
            this.result += "\\begin{lstlisting}\n";
            it.accept(this);
            this.result += "\\end{lstlisting}\n";

            this.result = this.result + "% Answer: \n\\begin{lstlisting}\n" +
                    question.getItemCombinationItems().get(i++).getOptions()
                            .stream()
                            .filter(ItemCombinationOption::isCorrect)
                            .sorted(Comparator.comparing(ItemCombinationOption::getSequence))
    ...
    public void visitItemCombinationItem(ItemCombinationItem item) {
        this.result += String.format("%n\t %s %n", item.getContent());
    ...
    public void visitItemCombinationOption(ItemCombinationOption option) {
        this.result += "\t\\itemCombinationOption{" + option.getContent() + "}\n";
    }
